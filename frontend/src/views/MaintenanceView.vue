<template>
  <AppLayout>
    <section class="maintenance-hero panel-card">
      ...
      <div class="form-actions"><el-button :loading="loading" @click="loadStatus">刷新状态</el-button></div>
    </section>
...
const actions = computed<MaintenanceAction[]>(() => [
  { key: 'history', title: '清除检测历史', desc: '清空检测记录数据库、上传文件和检测结果文件，保留 storage 目录结构。', button: '清除历史', confirm: '确认清除全部检测历史吗？', run: clearMaintenanceHistory },
  { key: 'logs', title: '清除日志', desc: '清空系统日志数据库和本地日志文件，保留日志目录结构。', button: '清除日志', confirm: '确认清除全部系统日志吗？', run: clearMaintenanceLogs },
  { key: 'models', title: '清除模型', desc: '清除非激活模型记录和 storage/models 下的非激活模型文件。', button: '清除模型', confirm: '确认清除全部非激活模型吗？', run: clearMaintenanceModels },
  { key: 'restore', title: '一键恢复初始化', desc: '恢复管理员账号、默认权限角色、系统基础字典和当前激活模型/默认模型。', button: '恢复初始化', confirm: '确认执行一键恢复初始化吗？', run: restoreInitialState },
])
...
function tagType(s: string): 'success' | 'danger' | 'warning' | 'info' { if (s === 'ok') return 'success'; if (s === 'error') return 'danger'; if (s === 'warning') return 'warning'; return 'info' }
