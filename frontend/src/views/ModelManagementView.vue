<template>
  <AppLayout>
    ...
    <section class="model-control-layout">
      <el-card shadow="never" class="panel-card device-control-panel">
        ...
        <div class="device-switcher">
          <el-button
            v-for="option in displayDeviceOptions" :key="option.value"
      ...
      <el-card shadow="never" class="panel-card model-register-card compact-form-card">
        ...
        <el-form :model="form" label-position="top">
          ...
          <div class="form-actions"><el-button type="primary" :disabled="!canManageModel || !canRegisterModel" @click="register">登记路径</el-button></div>
        </el-form>
    ...
    <el-card shadow="never" class="panel-card model-library-panel">
      <template #header><div class="toolbar"><span>模型库</span><el-button @click="load">刷新</el-button></div></template>
      <div class="table-scroll model-table-shell">
        <el-table :data="models" class="model-table">
          ...
          <el-table-column label="显示名称" min-width="150"><template #default="{ row }">{{ row.display_name || row.name }}</template></el-table-column>
          <el-table-column prop="name" label="后端名称" min-width="140" />
          <el-table-column prop="version" label="版本" width="120" />
          <el-table-column label="上次选择" width="120">
    ...
    <el-dialog v-model="displayDialog" title="修改模型显示名称" width="420px"><el-input v-model="displayNameForm" placeholder="请输入前端显示名称" /><template #footer><el-button @click="displayDialog = false">取消</el-button><el-button type="primary" @click="saveDisplayName">保存</el-button></template></el-dialog>

    <el-drawer v-model="mappingDrawer" title="类别中英文对照" size="54%">
      <div class="mapping-toolbar"><span>模型：{{ selectedModel?.display_name || selectedModel?.name }}</span><el-button type="primary" @click="saveMapping">保存映射</el-button></div>
...
const warmupText = computed(() => {
  ...
  return { cuda_ready: 'GPU 就绪', cpu_ready: 'CPU 就绪', pending: '等待初始化', failed: '初始化失败', not_loaded: '未加载', idle: '未初始化' }[status] || status
})
...
async function register() {
  ...
  ElMessage.success('模型路径已登记'); await load()
}
async function upload() {
  if (!uploadFileRef.value) return
  await uploadModel(uploadFileRef.value); uploadFileRef.value = null
  ElMessage.success('模型文件已上传'); await load()
}
async function switchDevice() {
  switchingDevice.value = true
  try { active.value = await switchModelDevice(selectedDevice.value); models.value = (await listModels()).items; ElMessage.success('推理设备已切换并预热') }
  finally { switchingDevice.value = false }
}
async function activate(id: number) {
  activatingId.value = id
  try { await activateModel(id, selectedDevice.value); ElMessage.success('模型已激活'); await load() }
...
async function saveDisplayName() {
  ...
  displayDialog.value = false; ElMessage.success('模型显示名称已更新'); await load()
}
...
async function saveMapping() {
  ...
  mappingDrawer.value = false; ElMessage.success('类别映射已保存'); await load()
}
