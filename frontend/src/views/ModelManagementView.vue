  ...
  <AppLayout>
    <el-card shadow="never" class="mb-s">
      <div class="flex-between">
        <div>
          <h2 style="margin-bottom:4px">{{ active?.active_model?.display_name || active?.active_model?.name || '尚未激活模型' }}</h2>
          <p>{{ active?.model_path || '请先上传或登记模型，然后选择推理设备并激活。' }}</p>
        </div>
        <div class="flex-gap">
          <div class="ops-info-item"><span>当前选择</span><strong>{{ deviceLabel(active?.requested_device || 'auto') }}</strong></div>
          <div class="ops-info-item"><span>实际运行</span><strong>{{ active?.device || '-' }}</strong></div>
          <div class="ops-info-item"><span>初始化</span><strong>{{ warmupText }}</strong></div>
          <div class="ops-info-item"><span>CUDA</span><strong>{{ active?.cuda_available ? active.cuda_name || '可用' : '不可用' }}</strong></div>
        </div>
      </div>
    </el-card>
    ...
    <div class="ops-panel cols-2">
      <el-card shadow="never">
        ...
        <div class="ops-info-item mb" :class="selectedDevice.startsWith('cuda') ? 'warn' : ''">
          <strong>{{ deviceLabel(selectedDevice) }}</strong>
          <span>{{ selectedDeviceDescription }}</span>
          <span v-if="selectedDeviceDetail">{{ selectedDeviceDetail }}</span>
        </div>
        ...
        <p v-if="active?.warmup_error" class="text-danger">{{ active.warmup_error }}</p>
      </el-card>
    ...
    <el-card shadow="never">
      <template #header>
        <div class="flex-between"><span>模型库</span><el-button size="small" @click="load">刷新</el-button></div>
      </template>
      <div class="ops-table-wrap">
        <el-table :data="models">
          ...
          <el-table-column prop="device" label="上次选择" width="110">
          ...
          <el-table-column prop="path" label="路径" min-width="240">
            <template #default="{ row }"><el-tooltip :content="row.path" placement="top"><span class="model-path-text">{{ row.path }}</span></el-tooltip></template>
          </el-table-column>
...
const warmupText = computed(() => ({ cuda_ready: 'GPU 就绪', cpu_ready: 'CPU 就绪', pending: '等待初始化', failed: '初始化失败', not_loaded: '未加载', idle: '未初始化' }[active.value?.warmup_status || 'idle'] || active.value?.warmup_status || '未初始化'))
const selectedDeviceDetail = computed(() => {
  const option = active.value?.available_devices.find((item) => item.value === selectedDevice.value)
  if (!option) return ''; if (selectedDevice.value.startsWith('cuda')) return `GPU 型号：${option.label || active.value?.cuda_name || selectedDevice.value}`
...
async function register() {
  const name = form.name.trim(); const path = form.path.trim(); const version = form.version.trim()
  if (!name || !path || !version) { ElMessage.warning('模型名称、路径和版本号不能为空'); return }
  await registerModel({ name, path, version, class_names: [] }); Object.assign(form, { name: '', path: '', version: '' }); ElMessage.success('模型路径已登记'); await load()
}
...
async function switchDevice() { switchingDevice.value = true; try { active.value = await switchModelDevice(selectedDevice.value); models.value = (await listModels()).items; ElMessage.success('推理设备已切换并预热') } finally { switchingDevice.value = false } }
...
async function saveDisplayName() { if (!selectedModel.value) return; await updateModelDisplayName(selectedModel.value.id, displayNameForm.value); displayDialog.value = false; ElMessage.success('模型显示名称已更新'); await load() }
...
async function remove(id: number) {
  try { await ElMessageBox.confirm('确认删除这个未激活模型吗？', '删除确认', { type: 'warning', confirmButtonText: '确认删除', cancelButtonText: '取消' }) } catch { return }
  await deleteModel(id); ElMessage.success('模型已删除'); await load()
}
