<template>
  <AppLayout>
    <section class="model-command-center">
    ...
    <section class="model-control-layout">
      ...
      <el-card class="workstation-panel" shadow="never">
        ...
        <div style="display: flex; gap: 10px; margin-top: 14px; padding-top: 14px; border-top: 1px solid var(--border-soft);">
          <el-upload :auto-upload="false" :limit="1" :on-change="selectUploadModel" style="flex: 1;">
            <el-button style="width: 100%;">选择 .pt 文件</el-button>
          </el-upload>
...
const selectedDeviceDescription = computed(() => {
  if (selectedDevice.value === 'auto') return '系统自动优先使用可用 GPU。'
...
async function remove(id: number) { try { await ElMessageBox.confirm('确认删除这个未激活模型吗？', '删除确认', { type: 'warning', confirmButtonText: '确认删除', cancelButtonText: '取消' }) } catch { return }; await deleteModel(id); ElMessage.success('模型已删除'); await load() }
