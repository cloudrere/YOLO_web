  ...
  <AppLayout>
    <el-card shadow="never" class="mb-s">
      <div class="flex-between">
        <div>
          <h2 style="margin-bottom:4px">YOLO results.csv</h2>
          <p>上传或选择 YOLO 训练生成的 results.csv，查看 Precision、Recall、mAP、Loss、学习率与 AI 训练建议。</p>
        </div>
        <div class="flex-gap">
          <el-tag size="small" :type="summary ? 'success' : 'info'">{{ summary ? '已加载摘要' : '等待 CSV' }}</el-tag>
    ...
    <div class="ops-panel cols-2-wide">
      <el-card shadow="never">
        ...
        <div class="flex-gap mb">
          ...
          <el-button size="small" type="danger" plain :disabled="!canManageTraining || !selectedName" @click="removeCurrentAnalysis">删除当前分析</el-button>
        ...
        <div class="ops-table-wrap" style="margin-top:8px">
      ...
      <div>
        <div class="ops-metrics cols-2" style="grid-template-columns: repeat(2, 1fr)">
          <div v-for="card in summaryCards" :key="card.label" class="ops-metric">
            <span class="metric-label">{{ card.label }}</span>
            <span class="metric-value text-mono" style="font-size:18px">{{ card.value }}</span>
            <span class="metric-desc">{{ card.desc }}</span>
          </div>
        </div>
        <el-card shadow="never">
          <template #header>自动风险提示</template>
          <div v-if="summary?.warnings.length" class="ops-queue">
            <div v-for="item in summary.warnings" :key="item" class="ops-queue-item" style="border-left:3px solid #e6a23c">{{ item }}</div>
          </div>
    ...
    <div class="ops-panel cols-2" style="margin-top:16px">
      <el-card shadow="never"><template #header>Precision / Recall 曲线</template><div ref="precisionRecallEl" v-loading="summaryLoading" class="ops-chart"></div></el-card>
      <el-card shadow="never"><template #header>mAP 曲线</template><div ref="mapEl" v-loading="summaryLoading" class="ops-chart"></div></el-card>
      <el-card shadow="never"><template #header>训练 / 验证 Loss 曲线</template><div ref="lossEl" v-loading="summaryLoading" class="ops-chart"></div></el-card>
      <el-card shadow="never"><template #header>最终指标雷达图</template><div ref="radarEl" v-loading="summaryLoading" class="ops-chart"></div></el-card>
      <el-card shadow="never"><template #header>最终 Loss 柱状对比</template><div ref="barEl" v-loading="summaryLoading" class="ops-chart"></div></el-card>
      <el-card shadow="never"><template #header>学习率曲线</template><div ref="lrEl" v-loading="summaryLoading" class="ops-chart"></div></el-card>
    </div>
...
const aiActionText = computed(() => { if (!summary.value) return '请先加载训练摘要'; if (!auth.hasPermission('assistant:use')) return '当前账号无 AI 助手权限'; if (!assistantStatus.value?.configured) return '后端未配置 AI_ASSISTANT_API_KEY'; return '可生成训练分析报告' })
...
async function exportReport() {
  if (!summary.value) return; const response = await exportTrainingReport(summary.value.name); const url = URL.createObjectURL(response.data); const link = document.createElement('a')
  link.href = url; link.download = `${summary.value.name.replace(/\.csv$/i, '')}_training_report.txt`; link.click(); URL.revokeObjectURL(url); ElMessage.success('训练分析报告已导出')
}
async function removeCurrentAnalysis() { if (!selectedName.value) return; try { await ElMessageBox.confirm(`确认删除当前训练分析 ${selectedName.value} 吗？`, '删除当前分析', { type: 'warning', confirmButtonText: '确认删除', cancelButtonText: '取消' }) } catch { return }; await deleteTrainingAnalysis(selectedName.value); ElMessage.success('当前训练分析已删除'); summary.value = null; aiReport.value = ''; selectedName.value = ''; disposeCharts(); await loadFiles() }
async function clearAllAnalyses() { if (!files.value.length) return; try { await ElMessageBox.confirm(`确认清空全部 ${files.value.length} 条训练分析吗？`, '清空全部分析', { type: 'warning', confirmButtonText: '确认清空', cancelButtonText: '取消' }) } catch { return }; await clearTrainingAnalyses(); ElMessage.success('全部训练分析已清空'); summary.value = null; aiReport.value = ''; selectedName.value = ''; files.value = []; disposeCharts() }
...
function renderPrecisionRecallChart() { const chart = makeChart(precisionRecallEl.value); const data = summary.value; if (!chart || !data) return; chart.setOption({ color: ['#409eff', '#e6a23c'], tooltip: { trigger: 'axis' }, legend: { top: 0 }, grid: chartBaseGrid(46), xAxis: { type: 'value', name: 'Epoch', min: 'dataMin', max: 'dataMax' }, yAxis: { type: 'value', max: 1 }, series: [lineSeries('Precision', data.precision), lineSeries('Recall', data.recall)] }) }
function renderMapChart() { const chart = makeChart(mapEl.value); const data = summary.value; if (!chart || !data) return; chart.setOption({ color: ['#409eff', '#67c23a'], tooltip: { trigger: 'axis' }, legend: { top: 0 }, grid: chartBaseGrid(46), xAxis: { type: 'value', name: 'Epoch', min: 'dataMin', max: 'dataMax' }, yAxis: { type: 'value', max: 1 }, series: [lineSeries('mAP50', data.map50, { areaStyle: { opacity: 0.12 } }), lineSeries('mAP50-95', data.map5095, { areaStyle: { opacity: 0.08 } })] }) }
function renderLossChart() { const chart = makeChart(lossEl.value); const data = summary.value; if (!chart || !data) return; chart.setOption({ color: ['#409eff', '#67c23a', '#e6a23c', '#f56c6c', '#909399', '#DDA0DD'], tooltip: { trigger: 'axis' }, legend: { top: 0, type: 'scroll' }, grid: chartBaseGrid(54), xAxis: { type: 'value', name: 'Epoch', min: 'dataMin', max: 'dataMax' }, yAxis: { type: 'value' }, series: [lineSeries('Train Box', data.train_box_loss), lineSeries('Train Cls', data.train_cls_loss), lineSeries('Train DFL', data.train_dfl_loss), lineSeries('Val Box', data.val_box_loss), lineSeries('Val Cls', data.val_cls_loss), lineSeries('Val DFL', data.val_dfl_loss)] }) }
function renderRadarChart() { const chart = makeChart(radarEl.value); const data = summary.value; if (!chart || !data) return; const values = data.radar.map((item) => Number(item.value) || 0); chart.setOption({ color: ['#409eff'], tooltip: {}, radar: { indicator: data.radar.map((item) => ({ name: String(item.name), max: 1 })), radius: '66%' }, series: [{ type: 'radar', areaStyle: { opacity: 0.18 }, data: [{ name: '最终指标', value: values }] }] }) }
function renderBarChart() { const chart = makeChart(barEl.value); const data = summary.value; if (!chart || !data) return; chart.setOption({ color: ['#e6a23c'], tooltip: { trigger: 'axis' }, grid: { left: 112, right: 18, top: 20, bottom: 28 }, xAxis: { type: 'value' }, yAxis: { type: 'category', data: data.bar_metrics.map((item) => item.name) }, series: [{ type: 'bar', barWidth: 20, data: data.bar_metrics.map((item) => Number(item.value) || 0) }] }) }
function renderLearningRateChart() { const chart = makeChart(lrEl.value); const data = summary.value; if (!chart || !data) return; chart.setOption({ color: ['#409eff', '#e6a23c', '#67c23a'], tooltip: { trigger: 'axis' }, legend: { top: 0 }, grid: chartBaseGrid(46), xAxis: { type: 'value', name: 'Epoch', min: 'dataMin', max: 'dataMax' }, yAxis: { type: 'value' }, series: [lineSeries('lr/pg0', data.lr_pg0), lineSeries('lr/pg1', data.lr_pg1), lineSeries('lr/pg2', data.lr_pg2)] }) }
