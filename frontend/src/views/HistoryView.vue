<template>
  <AppLayout>
    ...
    <el-drawer v-model="drawer" title="检测详情" size="64%">
      <div v-if="detail" class="ops-info-grid">
        <div class="ops-info-item"><span>用户</span><strong>{{ detail.username || detail.user_id || '暂无' }}</strong></div>
        <div class="ops-info-item"><span>模型</span><strong>{{ detail.model_name || '暂无' }}</strong></div>
        <div class="ops-info-item"><span>设备</span><strong>{{ detail.device || '暂无' }}</strong></div>
        <div class="ops-info-item"><span>检测时间</span><strong>{{ detail.created_at_text || detail.created_at }}</strong></div>
        <div class="ops-info-item"><span>参数</span><strong>{{ formatParameters(detail) }}</strong></div>
      </div>
      <div v-if="detail?.source_type === 'video' && (detail.video_url || detail.video_stream_url)" style="margin-bottom:12px">
      ...
      <div v-if="detail?.source_type !== 'video' && (detail?.original_url || detail?.result_url)" class="ops-compare">
...
async function exportRows() {
  ...
  link.href = url; link.download = '检测历史.xlsx'; link.click(); URL.revokeObjectURL(url)
