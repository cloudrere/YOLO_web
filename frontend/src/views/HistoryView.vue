  ...
  <AppLayout>
    ...
    <el-card shadow="never">
      <div class="filter-bar">
        ...
        <el-input v-model="filters.class_name" clearable placeholder="英文类别" style="width:130px" />
        <el-input v-model="filters.class_name_zh" clearable placeholder="中文类别" style="width:130px" />
      ...
      <div class="table-wrap">
        <el-table :data="rows" @row-click="openDetail" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="类别" min-width="180">
            <template #default="{ row }">
              <el-tag v-for="item in row.classes" :key="`${item.class}-${item.class_zh}`" type="success" size="small" style="margin:1px">
    ...
    <el-drawer v-model="drawer" title="检测详情" size="64%">
      ...
      <div v-if="detail?.source_type === 'video' && (detail.video_url || detail.video_stream_url)">
        <div class="preview-header" style="margin-bottom:8px"><h3>{{ detail.video_url && !localVideoFailed ? '本地检测后视频' : '检测帧流' }}</h3></div>
        <video v-if="detail.video_url && !localVideoFailed" :key="detail.id" :src="mediaUrl(detail.video_url)" controls preload="metadata" playsinline style="width:100%;max-height:400px" @error="handleVideoError"></video>
        <img v-else-if="detail.video_stream_url" :src="mediaUrl(detail.video_stream_url || '')" :alt="detail.file_name" style="width:100%" />
        <el-empty v-else description="暂无可播放视频" :image-size="40" />
      </div>
      <div v-if="detail?.source_type !== 'video' && (detail?.original_url || detail?.result_url)" class="compare-grid" style="margin-top:14px">
      ...
      <pre v-if="detail" class="json-dump">{{ JSON.stringify(detail, null, 2) }}</pre>
    </el-drawer>
  </AppLayout>
</template>
