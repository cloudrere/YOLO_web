    ...
    <div class="bi-workbench">
      ...
      <main class="bi-preview-stage">
        ...
        <div class="bi-preview-canvas">
          <div v-if="result?.items.length" class="bi-batch-grid">
            <img v-for="item in result.items.slice(0, 6)" :key="item.file_name" :src="mediaUrl(item.result_url)" :alt="item.file_name" />
          </div>
          <el-empty v-else :description="loading ? '批量任务处理中' : '批量检测完成后显示缩略图墙'" :image-size="60" />
        </div>
    ...
    <el-card shadow="never" class="bi-panel-card">
      <template #header>
        <div class="bi-card-header">
          <span class="bi-card-title">批量图片结果</span>
          <el-button size="small" :disabled="loading || !result" @click="clearResults">清除结果</el-button>
        </div>
      </template>
      <div v-if="result" class="bi-batch-result-grid">
        <article v-for="item in result.items" :key="item.file_name" class="bi-batch-result-card">
          <div class="bi-batch-compare">
          ...
          <div class="bi-batch-result-body">
            <div class="bi-card-header">
      ...
      <el-empty v-else description="批量检测完成后显示每张图片结果" :image-size="60" />
    </el-card>
  </AppLayout>
</template>
