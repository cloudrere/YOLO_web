    ...
    <el-card shadow="never" class="bi-panel-card">
      <template #header>
        <div class="bi-card-header">
          <span class="bi-card-title">系统日志</span>
          <el-button size="small" @click="load">刷新</el-button>
        </div>
      </template>
      <div class="bi-filter-bar">
        <el-select v-model="filters.level" clearable placeholder="级别" style="width:130px">
          <el-option label="信息" value="信息" />
          <el-option label="警告" value="警告" />
          <el-option label="错误" value="错误" />
          <el-option label="严重" value="严重" />
        </el-select>
        <el-input v-model="filters.module" clearable placeholder="模块" style="width:150px" />
        <el-input v-model="filters.log_type" clearable placeholder="日志类型" style="width:150px" />
