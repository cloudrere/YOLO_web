    ...
    <el-card class="workstation-panel" shadow="never">
      <template #header>
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <span style="font-weight: 700;">系统日志</span>
      ...
      <el-form :inline="true" :model="filters" class="filter-bar">
        ...
        <el-form-item label="模块"><el-input v-model="filters.module" clearable placeholder="检测 / 模型 / AI助手" style="width: 160px;" /></el-form-item>
        <el-form-item label="类型"><el-input v-model="filters.log_type" clearable placeholder="日志类型" style="width: 140px;" /></el-form-item>
        <el-form-item label="删除日期">
          <el-date-picker v-model="dateRange" type="daterange" value-format="YYYY-MM-DD HH:mm:ss" start-placeholder="开始日期" end-placeholder="结束日期" style="width: 260px;" />
        </el-form-item>
...
async function remove(id: number) {
  try { await ElMessageBox.confirm('确认删除这条日志吗？', '删除确认', { type: 'warning', confirmButtonText: '确认删除', cancelButtonText: '取消' }) } catch { return }
...
async function removeByDate() {
  ...
  const data = await deleteLogsByDate(dateRange.value[0], dateRange.value[1]); ElMessage.success(`已删除 ${data.deleted} 条日志`)
