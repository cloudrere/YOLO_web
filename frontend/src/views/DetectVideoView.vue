  ...
  <AppLayout>
    <div class="bi-page-header">
      ...
      <div class="bi-status-strip">
        <el-tag :type="task ? 'success' : 'info'" size="small">{{ task ? `任务 ${task.id}` : '未创建任务' }}</el-tag>
    ...
    <div class="bi-workbench">
      ...
      <main class="bi-preview-stage">
        ...
        <div class="bi-preview-canvas">
          ...
          <el-empty v-else description="创建视频任务后显示标注帧流" :image-size="60" />
        </div>
    ...
    <el-card shadow="never" class="bi-panel-card">
      <template #header>
        <div class="bi-card-header">
          <span class="bi-card-title">视频任务状态</span>
          <el-button size="small" :disabled="!task" @click="clearResults">清除结果</el-button>
        </div>
      </template>
      <div v-if="task">
      ...
      <el-empty v-else description="视频任务创建后显示进度和检测结果" :image-size="60" />
    </el-card>
  </AppLayout>
</template>
