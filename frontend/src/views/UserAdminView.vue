<template>
  <AppLayout>
    ...
    <div class="split-layout">
      ...
      <el-card shadow="never">
        <template #header><div class="flex-between"><span>用户列表</span><el-button size="small" @click="load">刷新</el-button></div></template>
        <div class="filter-bar">
          <el-input v-model="keyword" clearable placeholder="输入用户名关键词" style="width:220px" @keyup.enter="load" />
...
const roleNameMap: Record<string, string> = {
  admin: '管理员', operator: '操作员', user: '普通用户', viewer: '只读用户',
}
...
async function remove(id: number) {
  try { await ElMessageBox.confirm('确认删除这个用户吗？删除后该账号将无法继续登录。', '删除确认', { type: 'warning', confirmButtonText: '确认删除', cancelButtonText: '取消' }) } catch { return }
