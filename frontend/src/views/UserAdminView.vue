  ...
  <AppLayout>
    <section class="grid two">
      <el-card class="workstation-panel" shadow="never">
        <template #header><span style="font-weight: 700;">创建用户</span></template>
        <el-form :model="form" label-width="100px">
          ...
          <el-button type="primary" @click="create" style="width: 100%;">创建用户</el-button>
        </el-form>
      </el-card>
      <el-card class="workstation-panel" shadow="never">
        <template #header>
        ...
        <el-form :inline="true" class="filter-bar">
          <el-form-item label="用户查询">
            <el-input v-model="keyword" clearable placeholder="输入用户名关键词" style="width: 200px;" @keyup.enter="load" />
          </el-form-item>
...
async function create() {
  ...
  Object.assign(form, { username: '', password: '', is_active: true, is_superuser: false, role_ids: [] }); ElMessage.success('用户已创建'); await load()
}
async function toggleStatus(row: User) {
  ...
  await updateUser(row.id, { is_active: next }); ElMessage.success(`已${next ? '启用' : '停用'}`); await load()
}
...
async function remove(id: number) { try { await ElMessageBox.confirm('确认删除这个用户吗？', '删除确认', { type: 'warning', confirmButtonText: '确认删除', cancelButtonText: '取消' }) } catch { return }; await deleteUser(id); ElMessage.success('用户已删除'); await load() }
onMounted(load)
</script>
